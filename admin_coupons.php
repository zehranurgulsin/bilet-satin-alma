...
if(isset($_GET['del'])){
  $pdo->prepare("DELETE FROM coupons WHERE id=? AND company_id IS NULL")->execute([(int)$_GET['del']]);
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if($id){
    $pdo->prepare("UPDATE coupons SET code=?, discount_type=?, value=?, expires_at=?, usage_limit=?, min_price=?, active=? 
        ...
        ->execute([$code,$discount_type,$value,$expires_at,$usage_limit,$min_price,$active,$id]);
  }else{
    $pdo->prepare("INSERT INTO coupons (company_id, code, discount_type, value, expires_at, usage_limit, min_price, active)
                   VALUES (NULL,?,?,?,?,?,?,?)")
        ->execute([$code,$discount_type,$value,$expires_at,$usage_limit,$min_price,$active]);
  }
  header("Location: admin_coupons.php"); exit;
}

$rows=$pdo->query("SELECT * FROM coupons WHERE company_id IS NULL ORDER BY id DESC")->fetchAll();
$edit=null;
if(isset($_GET['edit'])){
  $st=$pdo->prepare("SELECT * FROM coupons WHERE id=? AND company_id IS NULL");
  $st->execute([(int)$_GET['edit']]); $edit=$st->fetch();
}
?>
<!doctype html><meta charset="utf-8"><title>Global Kuponlar</title>
<body style="font-family:system-ui;max-width:900px;margin:24px auto;">
<h2>Global Kuponlar (Tüm Firmalar)</h2>
...
<form method="post" style="border:1px solid #ddd;padding:10px;border-radius:8px;margin:8px 0;display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px;">
  ...
  <input name="code" placeholder="Kod" value="<?=htmlspecialchars($edit['code']??'')?>" required>
